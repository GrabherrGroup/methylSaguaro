...
int main( int argc, char** argv )
{
  ...
  commandArg<int> allCmmd("-iter","iterations with split", 12);
  commandArg<int> stateCmmd("-state","Number of states to use", 3);
  commandArg<double> transCmmd("-t","transition penalty", 40.);
  ...
  P.SetDescription("Saguaro adapted for Methylation data");
